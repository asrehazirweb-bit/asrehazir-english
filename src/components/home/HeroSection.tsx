...
export function HeroSection({ leadStory, className = "" }: HeroSectionProps) {
    return (
        <Link to={`/news/${leadStory.id}`} className={`relative w-full block mb-16 group cursor-pointer border-b border-gray-100 dark:border-white/5 pb-12 transition-all duration-500 hover:shadow-2xl hover:shadow-red-500/5 ${className}`}>
            {/* Image Container */}
            <div className="relative aspect-[21/10] w-full overflow-hidden mb-10 bg-gray-100 dark:bg-white/5 rounded-xl shadow-inner">
                <img
                    src={leadStory.image}
                    alt={leadStory.title}
            ...
            <div className="flex flex-col gap-6 max-w-5xl px-2">
                ...
                <h1 className="font-serif font-black text-4xl md:text-6xl lg:text-7xl leading-[1.05] text-gray-900 dark:text-gray-50 group-hover:text-red-700 transition-colors duration-400 tracking-tight">
                    {leadStory.title}
                </h1>

    ...
    return (
        <div className="mb-12">
            <div className="flex items-center mb-6 border-b border-accent/30 pb-2">
                ...
                <div className="flex-grow ml-4 h-[1px] bg-gray-100 dark:bg-white/10"></div>
            </div>
                ...
                {items.map((item) => (
                    <Link key={item.id} to={`/news/${item.id}`} className="group cursor-pointer block">
                        <div className="relative overflow-hidden mb-3 aspect-video bg-gray-200 dark:bg-white/5">
                            ...
                            />
                            <span className="absolute bottom-0 left-0 bg-accent text-white text-[10px] uppercase font-bold px-2 py-1">
                                {item.category}
                            </span>
                        </div>

...
const AdminLayout: React.FC = () => {
    ...
    const navItems = [
        ...
        { path: '/admin/categories', label: 'Categories', icon: <FolderOpen size={18} /> },
        { path: '/admin/ads', label: 'Ads Manager', icon: <Megaphone size={18} /> },
        ...(userData?.role === 'admin' ? [
            { path: '/admin/users', label: 'Admin Requests', icon: <Zap size={18} /> },
            { path: '/admin/settings', label: 'App Settings', icon: <Settings size={18} /> }
        ] : []),
    ];

    return (
        <div className="flex h-screen bg-white transition-colors duration-500 overflow-hidden" dir="ltr">
            {/* Mobile Header */}
            <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-100 flex items-center justify-between px-6 z-30">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-black text-gray-900 uppercase tracking-tighter">Asre Hazir</span>
                </div>
                <button
            ...
            <aside className={`
                fixed inset-y-0 left-0 z-40 w-72 bg-white border-r border-gray-100 flex flex-col shadow-xl transition-transform duration-300 transform
            ...
            `}>
                <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-black text-gray-900 uppercase tracking-tighter leading-none">Asre Hazir</h2>
                            <span className="text-[10px] font-black text-primary uppercase tracking-widest">Control Center</span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <nav className="space-y-1.5">
                            <Link
                                to="/"
                                onClick={() => setIsMobileSidebarOpen(false)}
                                className="flex items-center space-x-3 px-4 py-3.5 rounded-2xl text-gray-500 hover:bg-gray-50 hover:text-gray-900 transition-all group"
                            >
                ...
                <div className="p-6 border-t border-gray-50 bg-gray-50/50">
                    <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="overflow-hidden">
                                <p className="text-sm font-black text-gray-900 truncate uppercase tracking-tight">{auth.currentUser?.displayName || 'Admin'}</p>
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center space-x-2 p-3.5 rounded-2xl bg-gray-100 text-gray-600 hover:bg-primary/10 hover:text-primary transition-all font-black uppercase tracking-widest text-[10px]"
                    >
                        <LogOut size={16} />
                ...
                <div className="max-w-6xl" style={{ marginInline: 'auto' }}>
                    ...
                    <div className="flex items-center justify-between mb-8 pb-6 border-b border-gray-200/50">
                        <div className="flex items-center gap-2 text-[10px] uppercase font-black tracking-[0.2em] text-gray-400">
                            ...
                            <span className="text-gray-900 italic">{location.pathname.split('/').pop()?.replace('-', ' ') || 'Dashboard'}</span>
                        </div>

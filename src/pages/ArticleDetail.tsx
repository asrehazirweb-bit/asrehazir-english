...
const ArticleDetail: React.FC = () => {
    ...
    if (error || !article) {
        return (
            <div className="flex flex-col justify-center items-center min-h-screen bg-white px-4">
                <h2 className="text-2xl font-serif font-bold text-gray-900 mb-4">{error || 'Article not found'}</h2>
    ...
    return (
        <article className="bg-white transition-colors duration-300 min-h-screen pb-20">
            {/* Progression Bar (Standard for premium blogs) */}
            <div className="hidden md:block fixed top-0 left-0 w-full h-1 bg-gray-100 z-50">
                <div className="h-full bg-primary w-0 transition-all duration-300" id="reading-progress"></div>
            </div>

            {/* Header / Breadcrumb */}
            <div className="max-w-4xl mx-auto px-4 pt-8 pb-4">
                <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] font-black text-primary mb-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-6 pb-8 border-b border-gray-100">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="text-sm font-black text-gray-900 uppercase tracking-wider">{article.author || 'Asre Hazir Desk'}</div>
            ...
            <div className="max-w-6xl mx-auto px-0 md:px-4 mb-12">
                <div className="relative w-full bg-gray-100 md:rounded-3xl overflow-hidden shadow-2xl">
                    {article.imageUrl ? (
                        <>
                            <img
            ...
            {relatedNews.length > 0 && (
                <div className="max-w-6xl mx-auto px-4 mt-24">
                    <div className="flex items-center justify-between mb-10">
                        <h2 className="text-2xl font-serif font-black text-gray-900 uppercase tracking-tight italic underline decoration-primary decoration-4 underline-offset-8">
                            More from {article.category}
                        </h2>
                        <Link to={getCategoryPath(article.category)} className="text-xs font-black uppercase tracking-[0.2em] text-primary hover:text-black transition-colors">
                            View All Stories
                        </Link>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {relatedNews.slice(0, 3).map((item) => (
                            <Link key={item.id} to={`/news/${item.id}`} className="group block">
                                ...
                                <h3 className="font-serif font-bold text-lg text-gray-900 group-hover:text-primary transition-colors line-clamp-2 leading-tight">
                                    {item.title}
                                </h3>
